...
var apiRequest = function(apiKey, options, callback, bypassCache) 
{
	...
	if (bypassCache || typeof cache.get(apiKey) === 'undefined' || (new Date()) > cache.get(apiKey).updateAt) 
	{
		...
		if (config.debug) 
			console.log('Updating cache for API Key: ' + apiKey + ' from URL: ' + url);
	...
	var ret = {
		...
		setCacheTime: function(seconds, apiKey) 
		{
			...
			if (apiKey === null) 
			{
				...
				if (config.debug) 
					console.log('setCacheTime successful; config.api: ' + JSON.stringify(config.api));
			...
			else if (!(apiKey in config.api)) 
			{
				...
				if (config.debug) 
					console.log('setCacheTime successful; config.api.' + apiKey + ': ' + JSON.stringify(config.api[apiKey]));
			}
